...
import com.dt.xfin.parser.JBig2Image;
...
public class PDFTextStripper extends PDFStreamEngine
{
	...
	private Boolean processedImages =false;
	private List<String> imageSources=new ArrayList<String>();

	private List<PDThreadBead> pageArticles = null;
	
	//JBIG2 global infomration for jbig2 decoder. each is map by page
	private Map<Integer, byte[]> jBIG2_Global_map=new HashMap<Integer, byte[]>();
	/**
	...
	protected void processPages( List<COSObjectable> pages ) throws IOException
	{
		...
		//populate the bigj2globals, used by JPedal Bigj2 decoder to decode the image
		while( pageIter.hasNext() )
		{
			...
			currentPageNo++;
				...
				PDResources resources=nextPage.findResources();
				...
				{	
					for(String key:images.keySet())
					{
						...
						//int bits= images.get(key).getBitsPerComponent();
						COSDictionary decodeP =(COSDictionary)images.get(key).getCOSStream().getDictionaryObject( COSName.DECODE_PARMS );
				        COSStream st = null;
				        
				        InputStream jBIG2_Global_input=null;
				        ...
				        if(st != null)
				        {
				        	jBIG2_Global_input=st.getFilteredStream();
				        	byte[] JBIG2GlobalsBytes=new byte[jBIG2_Global_input.available()];
				    		jBIG2_Global_input.read(JBIG2GlobalsBytes);
				    		jBIG2_Global_input.close();
				    		
				        	jBIG2_Global_map.put(currentPageNo, JBIG2GlobalsBytes);
				        }
		...
		//reset the page number and iterator
		currentPageNo=0;
	...
	protected void processPage( PDPage page, COSStream content ) throws IOException
	{
				...
				(endBookmarkPageNumber == -1 || currentPageNo <= endBookmarkPageNumber ))
		{
			...
			//go ahead add image to the page
			...
			{	
				...
				//jBig2Image will parse whole pdf and save file with pagenumber and key
			        if(!processedImages)
			        {
			        	new JBig2Image(MultimediaParser.getCurrentFilePath(), 
							MultimediaParser.getCurrentOutputFolder(),jBIG2_Global_map);
					
			        	processedImages=true;

			        }
				//go ahead write the current page image
				//imageFile=MultimediaParser.getCurrentOutputFolder() + File.separator
				//	+currentPageNo+"_"+key;

			        //reset the image source
			        imageSources.clear();
				//get image for this page
				List<String> imagesForThisPage= JBig2Image.getImageSourceMaps().get(currentPageNo);
				if(imagesForThisPage!=null)	

				{
					for(String im: imagesForThisPage)
					{
						String imageFile=MultimediaParser.getUrlOutputFileName(im);
						//imageSources.add(imageFile);
						imageSources.add(dataImporter.encode(imageFile));
					}
			...
			processStream( page, resources, content );
			writePage();
			endPage( page );
		}
	...
	protected void writePage() throws IOException
	{
		...
		for( int i = 0; i < charactersByArticle.size(); i++)
		{
			...
			if (line.size() > 0)
			{
				List<String> normalized=normalize(line,isRtlDominant,isRtlDominant);
				for(String st:normalized)
				{
					content+='\n'+st;
				}
			...
			PDFData pdfPage=new PDFData();
			if(dataImporter.isExtract_individual())
			{
				pdfPage.setPageNumber(this.currentPageNo);
				pdfPage.setContent(content);
				pdfPage.setImage_links(new ArrayList<String>(imageSources));
			
				MultimediaParser.getCurrentContents().add(pdfPage);
			}
